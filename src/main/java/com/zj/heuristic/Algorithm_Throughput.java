...
public class Algorithm_Throughput {
	 ...
	 * @return
	 */
	public static int[][][] getAllpathsByAllDemand(Demands demands,Topology topo) {
		// 路径集根据最短优先级已排序
		int[][][] allpaths = new int[demands.demandsCount()][][];
		for (int i = 0; i < demands.demandsCount(); i++) {
			allpaths[i] = AllpathSolver.getAllpath(topo, demands.getDemand(i+1).getSrcNoed(), demands.getDemand(i+1).getDestNode(), demands.getMaxDelayRate());
		}
		return allpaths;
	}
	
	private static boolean isInstall(boolean[] installedDemands) {
		for (int i = 0; i < installedDemands.length; i++) {
			if (!installedDemands[i]) {
				return false;
			}
		}
		return true;
	...
	public static Solution installNFC(Topology topo, Demands demands) {
		//需求集根据SFC长度排序
		ArrayList<Demand> dems = new ArrayList<>(demands.getDemands());
		Collections.sort(dems);
		Solution solu = new Solution("T_Solu", demands.demandsCount());
		//所有需求的路径集
		int[][][] allpaths = getAllpathsByAllDemand(demands, topo);
		//初始化需要节点的VM
		createVMbyPDH(topo, solu);
		//待接纳的需求集
		boolean[] installedDemands = new boolean[demands.demandsCount()];
		int k = 0;//路径的迭代次数，
		while (!isInstall(installedDemands)) {
			for (Demand dem : dems) {
				//请求的数组序号
				int j = dem.getId()-1;
				if (installedDemands[j]) {
					continue;
				}else if (k >= allpaths[j].length) {
				//如果该dem的路径已经遍历完
					installedDemands[j] = true;
					break;
				}else {
					for (int i = 0; i <= dem.SFCCount(); i++) {
						// 生成指定路径上的预安装NFC
						LinkedHashMap<NFtype, Node> preNFC = updatePreNFC(i, topo, solu, dem, allpaths[j][k]);
						// TODO:判断该路径是否可以安装NFC
						if (preNFC.size() == dem.SFCCount()) {
							// TODO:可以安装
							createFlow(topo, solu, dem, allpaths[j][k], preNFC);
							installedDemands[j] = true;
							break;	
						}
					}
	...
	public static LinkedHashMap<NFtype, Node> updatePreNFC(int numPreSFs, Topology topo, Solution solu, Demand dem,
			int[] path) {
	...
	public static void updateNFC(Solution solu, Topology topo, Demands demands) {
		for (Demand dem : demands.getDemands()) {
			...
			if (!dem.getIsInstall()) {
				// 路径集根据最短优先级已排序
				int[][] paths = AllpathSolver.getAllpath(topo, dem.getSrcNoed(), dem.getDestNode(),
						demands.getMaxDelayRate());
	...
	/*
	 * public static LinkedHashMap<NFtype, Node> createPreNFC(Solution solu, Demand
	 * dem, int[] path){ int numSF = dem.SFCCount(); LinkedHashMap<NFtype, Node>
	 * preNFC = new LinkedHashMap<NFtype, Node>(numSF); int index = 1; for(int i=0;
	 * i<path.length; i++){ //注意vm为空 VM vm = solu.getVM(path[i]); if(vm==null ||
	 * !vm.hasVM()){ continue; }
	 *
	 * //在一个节点的VM中最多需要找numSF次 for(int j=1; j<=numSF; j++){ NFtype nf =
	 * dem.getNFInSFC(index); if (checkSFInVM(dem, nf, vm, preNFC)) { index++; if
	 * (index > numSF) { return preNFC; } }else { break; } } } //没有被覆盖的情况
	 ...
	 * @param solu
	 */
	private static void createVMbyPDH(Topology topo, Solution solu) {
		for (Node node : topo.getNodes()) {
			solu.newVM(node);
		}
		topo.getNode(0).setBanFlag(true);
		topo.getNode(2).setBanFlag(true);
		topo.getNode(3).setBanFlag(true);
		topo.getNode(4).setBanFlag(true);
		topo.getNode(6).setBanFlag(true);
		topo.getNode(9).setBanFlag(true);
		topo.getNode(10).setBanFlag(true);
	}
