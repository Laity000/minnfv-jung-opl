 ...
 * OPL 12.7.0.0 Model
 * Author: PC
 * Creation Date: 2017年10月30日 at 下午10:42:29
 *********************************************/
...
minimize 100 * TotalAcitviedNodeMun + TotalFuncMun + MaxDelay/1000;
subject to {
...
//	  	TotalFunc[i][m] / NumVM <= ActivedNode[i];
	...
	// delay constraints
	  ctdelay:
	     sum (dem in Demands, a in Arcs) Flow[dem.id][a] * dem.supply <= MaxDelay;
