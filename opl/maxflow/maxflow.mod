 ...
 * OPL 12.7.1.0 Model
 * Author: zj
 * Creation Date: 2019年1月23日 at 下午4:29:54
 *********************************************/
...
dexpr float TotalThrought =  sum (dem in Demands) X[dem.id] * dem.supply;
maximize TotalThrought - 0.001* MaxDelay;
...
subject to {
    ...
    == sum (dem in Demands : dem.id == k && dem.srcnode == i) X[k]
    - sum (dem in Demands : dem.id == k && dem.destnode == i) X[k];
	...
	// node's VM num constraints
	forall (i in Nodes)
	  ctNodeResourse:
	    sum (m in NFs) TotalFunc[i][m] <= NumVM[i];
...
//	  	TotalFunc[i][m] / NumVM <= ActivedNode[i];
	...
	forall(k in Dems, i in Nodes)
	  ctFlowOrdinalLower:
	    FlowOrdinal[k][i] <= sum (<i,j,c> in Arcs) X[k]*NumNodes*Flow[k][<i,j,c>] + sum (<j,i,c> in Arcs) X[k]*NumNodes*Flow[k][<j,i,c>];
	
	// flowordinal origin constraints, origin node is NO.1
	
	forall(dem in Demands, i in Nodes : dem.srcnode == i)
	  ctFlowOrdinalOrigin:
	   FlowOrdinal[dem.id][i] == X[dem.id];
	//////////////////////////////
	
	// delay constraints
	  ctdelay:
	     sum (dem in Demands, a in Arcs) Flow[dem.id][a]  <= MaxDelay;
	...
	forall (dem in Demands, m in NFs : dem.NFC[m] > 0)
	  ctNFCUpperNum:
	    sum (i in Nodes) Func[dem.id][i][m] == X[dem.id];
...
execute DISPLAY {
   ...
   write("\nActivedFlow<dem,supply>\n");
   for (var dem in Demands)
     if (X[dem.id] > 0)
       write("<",dem.id,",",dem.supply,">\n");
